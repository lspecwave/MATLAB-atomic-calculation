...
By applying simultaneous Ramsey interferometry on both isotopes, we determine the precession phases, and derive the Larmor frequencies of $^{171}$Yb ($\omega_{171}$) as well as the cat state of $^{173}$Yb ($\omega_{173}$), both with a statistical sensitivity of $\sim\ 2\pi\times2$ mHz in a single-shot measurement, primarily limited by spin-projection noise and residual light-shift fluctuations from the optical trap.

Under ideal conditions, the comagnetometer frequency ratio $R_L = \omega_{171}/\omega_{173}$ is insensitive to magnetic-field fluctuations. However, we find that light shifts induced by the high-power optical lattice could cause a significant perturbation to $R_L$.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l l}
        \toprule
        \textbf{Measurement} & $\mathbf{R_{NMM}}$ \\
        \midrule

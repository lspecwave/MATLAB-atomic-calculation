...
By applying simultaneous Ramsey interferometry on both isotopes, we determine the precession phases, and derived the Larmor frequencies of $^{171}$Yb ($\omega_{171}$) as well as the cat state of $^{173}$Yb ($\omega_{173}$), both with a statistical sensitivity of $\sim\ 2\pi\times2$ mHz in a single-shot measurement.
...
\section*{Dependence on light shift}
...
\subsection*{Tensor shift}

Tensor shifts are naturally absent in the ground state $^{1}\mathrm{S}_0$ of $^{171}$Yb (I = 1/2). For $^{173}$Yb (I = 5/2), tensor shifts are also expected to be null in the cat state of $^{1}\mathrm S_0$, but instead are found in this work to affect the Larmor frequency in a significant and peculiar way. The tensor operator aligns along the linear polarization direction (z) of the lattice, which, due to imperfection, form a small angle $\theta$ with the bias magnetic field $\mathrm B$ in the Z-direction. Without loss of generality, assume the magnetic field has a small component in the transverse x-direction, the Hamiltonian $\mathscr H$ arising from the combined magnetic and tensor fields can be expressed as
...
\end{equation}
From 50 mG to 100 mG, the magnetic field changes by a factor of two, so are the Larmor frequencies of each isotope, yet the comagnetometer ratio $R_L$ remains constant at the 10 ppm level. We derive the suppression factor $\eta>3\times10^4$ ($95\%$ confidence level) based on the two $R_L$ values and their corresponding uncertainties. %This is equivalent to a 90 dB reduction in the magnetic-field noise power.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Fig2.pdf}
    \caption{The combined effect of Zeeman and tensor shifts on the Larmor frequency ($\omega_{173}$) of the cat state of $^{173}$Yb, and on the comagnetometer ratio $R_L=\omega_{171}/\omega_{173}$. $^{173}$Yb atoms are populated in $\left\vert5/2,\pm5/2\right\rangle_g$ (green dots). 
...
\subsection*{Residual vector shift}
...
\section*{Ratio of nuclear magnetic moments}

We have investigated systematic uncertainties in the measurements of the comagnetometer ratio $R_L$. After aligning the 100 mG magnetic field to be parallel to the polarization direction of the optical lattice, we measure $R_L$ while varying the lattice power by more than a factor of five. No dependence of $R_L$ on light power is observed within the measurement uncertainties (Fig.~\ref{fig3}a). A weighted average across different lattice powers yields $R_L = 0.726077(2)$.
...
We calculate a weighted average of comagnetometer ratio  $R_L$ across different experimental parameters, yielding a final value of $R_L = 0.726076(3)$ and the ratio of nuclear magnetic moments between $^{171}$Yb and $^{173}$Yb, $R_{NMM} = \mu_{I,171}/\mu_{I,173} = -0.726076(3)$. This new result agrees with the recommended value of $-0.72610(4)$ \cite{Gossard1964RefNMR, Olschewski1972Ref, Stone2005Ref}, while improving the precision by an order of magnitude.
...
%This cold-atom comagnetometer, by leveraging the advantage of localized cold atoms, significantly reduces the detection distance while maintaining its capability to suppress magnetic field noise. This enables probing of subtle signals such as investigation of new physics beyond the Standard Model \cite{Safronova2018NewPhys}, within a hundred-micron or even micron scale force measurement range. As illustrative examples, we are pursuing the following two research directions.

The comagnetometer is used to search for $\mathit P$- and $\mathit T$-violating spin-mass couplings, also known as monopole-dipole interactions \cite{Moody1984M-N, Bulatowicz2013ComagAxionlike, Tullney2013SpinDependent, Feng2022XeComagM-N}. In this search, with $^{171}$Yb and $^{173}$Yb atoms as spin sources, the comagnetometer ratio is monitored as a dense mass source is brought into proximity with the atoms. By using optical tweezers to position the atoms, the atom-mass separation can be precisely controlled down to a few micrometers. We estimate that this setup can improve the sensitivity to spin-mass couplings at the 1 $\mu$m interaction range by up to three orders of magnitude compared to the current experimental limits \cite{Guigue2015Spin3HePRD}.


%This comagnetometer is able to probe spin-mass couplings violating $\mathit P$ and $\mathit T$ symmetries, also known as monopole-dipole interactions \cite{Moody1984M-N}. We employ $^{171}$Yb and $^{173}$Yb atoms as spin sources, varying their separation from a mass source while measuring the precession frequency ratio between these spins at different distances. In our experimental design, optical tweezers restricts atomic locations, enabling an atom-mass separation down to several micrometers under vacuum conditions. According to estimates, sensitivity to spin-mass couplings at a 1-$\mu$m interaction range will be enhanced by three orders of magnitude \cite{Guigue2015Spin3He}.

We aim to improve the search sensitivity of electric dipole moments (EDMs) using cold atoms \cite{Bishof2016RaEDM, Zheng2022YbEDM}. The current EDM experiments on $^{171}$Yb \cite{Zheng2022YbEDM} are limited by magnetic-field noises, particularly the Johnson noise in adjacent electrodes. A $^{171}$Yb-$^{173}$Yb cold-atom comagnetometer can effectively suppress such noise effects, thereby boosting the EDM search sensitivity. Furthermore, EDM searches have been performed on cold atoms of $^{225}$Ra (I = 1/2), an isotope whose octupole-deformed nucleus provides enhanced sensitivity to $\mathit T$-violating interactions \cite{Bishof2016RaEDM}. As a future direction, we propose forming a comagnetometer using $^{223}$Ra (I = 3/2) and $^{225}$Ra to suppress magnetic noise in the next-generation $^{225}$Ra EDM experiments.

%Additionally, this cold-atom comagnetometer will play a significant role in EDM measurements. In current EDM detection in $^{171}$Yb cold atoms, measurement uncertainties are primarily limited by magnetic field noise, particularly Johnson noise induced by adjacent high-voltage electrodes. We plan to implement a $^{171}$Yb-$^{173}$Yb cold-atom comagnetometer to mitigate magnetic noise effects, thereby enhancing the EDM measurement precision for $^{171}$Yb atoms. Furthermore, cold-atom system in optical traps has been employed to measure the EDM of $^{225}$Ra (I = 1/2), which is significantly enhanced by the octupole deformation of the $^{225}$Ra nucleus \cite{Bishof2016RaEDM}. We propose to use the $^{223}$Ra isotope (nuclear spin I = 3/2) paired with $^{225}$Ra to form a comagnetometer, eliminating magnetic field noise during measurements.
...
%Eq.~\ref{perturbation2nd} describes the Hamiltonian of $^{173}$Yb on both the Zeeman shifts induced by the bias magnetic field and the tensor light shifts by the optical lattice. It can be seen that $\left\vert5/2,+5/2\right\rangle_g$ consistently remains the highest-energy state. The eigenenergy of $\left\vert5/2,-5/2\right\rangle_g$ can be lower or higher than the neighboring $\left\vert5/2,-3/2\right\rangle_g$ state, depending on the ratio $\Omega_{\mathrm B}/\Omega_{\mathrm T}$. At $\Omega_{\mathrm B}/\Omega_{\mathrm T}=4.0$, the eigenenergies of $\left\vert5/2,-5/2\right\rangle_g$ and $\left\vert5/2,-3/2\right\rangle_g$ exhibit degeneracy.
...
The perturbated eigenenergies are
...
%Among the measured sublevels $\left\vert^{1}\mathrm{S}_0,-5/2\right\rangle$ and $\left\vert^{1}\mathrm{S}_0,+5/2\right\rangle$, only $\left\vert^{1}\mathrm{S}_0,-5/2\right\rangle$ may couple exclusively with $\left\vert^{1}\mathrm{S}_0,-3/2\right\rangle$ due to the subdiagonal matrix structure. As shown in Eq.~\ref{perturbation2nd}, on the opposite sides of the critical ratio $\Omega_{\mathrm B}/\Omega_{\mathrm T}=4.0$, the sign of $\Delta E^{(2)}$ reverses.


\subsection*{Calibration of $\Omega_{\mathrm B}/\Omega_{\mathrm T}$ }

We perform precession measurements on $^{173}$Yb to calibrate the $\Omega_{\mathrm B}/\Omega_{\mathrm T}$ values. Since the cat state is immune to tensor shifts ($\Omega_{\mathrm T}$), we employ a coherent spin state (CSS) instead, whose spin is fully polarized along a direction.

%To obtain the $\Omega_{\mathrm B}/\Omega_{\mathrm T}$ value in Fig.~\ref{fig2}, we performed precession measurements on $^{173}$Yb. Since the cat state is immune to tensor shifts, we employed the coherent spin state (CSS) instead.

While keeping the lattice linearly polarized along the z-axis, its power is reduced to 8 W. Meanwhile, a bias magnetic field of 23.5 mG is applied along the x-direction. The $^{173}$Yb atoms are first pumped into $\left\vert5/2,+5/2\right\rangle_g$, then allowed to precess under the influence of both the Zeeman and tensor shifts, $\mathscr{H}_{calib} = \Omega_{\mathrm B}\sigma_x + \Omega_{\mathrm T}\sigma_z^2$. After a variable precession duration (0 – 2,500 ms), the population signal $S_z$ is measured along the z-axis (Fig.~\ref{fig4}). 

%We maintain the optical field unchanged (still linearly polarized along the z-axis), only reduce the power to 8 W. Meanwhile, we reorient the bias magnetic field along the x-direction (23.5 mG). Under these conditions, we first pumped the $^{173}$Yb atoms along the z-axis to generate a CSS $\left\vert5/2,+5/2\right\rangle_g$. The atoms subsequently undergo Larmor precession around the x-direction bias magnetic field, while simultaneously experiencing the z-direction tensor light shift. After a variable precession period (0–2,500 ms), we perform detection along the z-axis to obtain the $S_z$ signal (Fig.~\ref{fig4}). The resulting precession curve is governed by the following Hamiltonian:

%\begin{equation}
%    \mathscr{H}_{\mathrm test} = \Omega_{\mathrm B}\sigma_x + \Omega_{\mathrm T}\sigma_z^2
%\end{equation}

We simulate the precession process of $^{173}$Yb using the calibration Hamiltonian ($\mathscr{H}_{calib}$) and compare the results with experimental data, obtaining $\Omega_{\mathrm B}/\Omega_{\mathrm T}=3.5$ under the given magnetic field and lattice intensity conditions.

%We simulated the precession process of $^{173}$Yb using this Hamiltonian and compared it with experimental data, obtaining $\Omega_{\mathrm B}/\Omega_{\mathrm T}=3.5$ under the given magnetic field and light intensity conditions.
...
\subsection*{Atom sample detection}

A circularly polarized laser probe beam along the Z-direction runs through the cold atom ensemble and is detected by a CMOS camera. Absorption images are taken on the atoms for population measurements. The state readout of atoms is performed via differential detection of two stretched states. For $^{171}$Yb atoms, the probe beam resonantly excites the cycling transition $\left\vert1/2,+1/2\right\rangle_g \rightarrow \left\vert{3/2},+3/2\right\rangle_e$ (Fig.~\ref{fig5}a). Meanwhile, the $\left\vert1/2,-1/2\right\rangle_g \rightarrow \left\vert3/2,+1/2\right\rangle_e$ transition is off resonance by 10 times the natural linewidth ($10\ \Gamma$) due to tensor shifts on the excited states. As a result, the cycling transition can be repeated 1,200 times on average before a spin flip occurs. This spin selectivity ensures that the spin state is detected with an efficiency approaching $100\%$. For $^{173}$Yb atoms, the relevant transitions are $\left\vert5/2,+5/2\right\rangle_g \rightarrow \left\vert7/2,+7/2\right\rangle_e$ and $\left\vert5/2,-5/2\right\rangle_g \rightarrow \left\vert3/2,-3/2\right\rangle_e$ (Fig.~\ref{fig5}b). In addition to a comparable difference of tensor shifts ($\sim20\ \Gamma$), the branching ratios of these two transitions differ by a factor of 21, thus further enhancing the spin selectivity.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{Fig5.pdf}
    \caption{
     Transitions used to measure spin-state populations in $^{171}$Yb (a) and $^{173}$Yb (b). The arrow of solid line indicates a cycling transition used for population measurements; the arrow of dashed line indicates a spin-flip transition suppressed by large detunings.}
     \label{fig5}
\end{figure}
...
\newblock \emph{\bibinfo{journal}{Reviews of Modern Physics}} \textbf{\bibinfo{volume}{90}}, \bibinfo{pages}{035005} (\bibinfo{year}{2018}).
...
\newblock \emph{\bibinfo{journal}{Nature Physics}} \textbf{\bibinfo{volume}{3}}, \bibinfo{pages}{227–234} (\bibinfo{year}{2007}).
...
\newblock \emph{\bibinfo{journal}{Reviews of Modern Physics}} \textbf{\bibinfo{volume}{97}}, \bibinfo{pages}{025005} (\bibinfo{year}{2025}).
...
\newblock \bibinfo{title}{Measurement of the \rm{E}lectric \rm{D}ipole \rm{M}oment of $^{171}\rm{Yb}$ atoms in an \rm{O}ptical \rm{D}ipole \rm{T}rap}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{129}}, \bibinfo{pages}{083001} (\bibinfo{year}{2022}).
...
\newblock \emph{\bibinfo{journal}{Quantum Science and Technology}} \textbf{\bibinfo{volume}{7}}, \bibinfo{pages}{014001} (\bibinfo{year}{2021}).
...
\newblock \bibinfo{title}{Limits on \rm{N}ew \rm{L}ong \rm{R}ange \rm{N}uclear \rm{S}pin-\rm{D}ependent \rm{F}orces \rm{S}et with a $\mathrm{K}\mathrm{\text{\ensuremath{-}}}^{3}\mathrm{He}$ \rm{C}omagnetometer}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{103}}, \bibinfo{pages}{261801} (\bibinfo{year}{2009}).
...
\newblock \emph{\bibinfo{journal}{Annalen der Physik}} \textbf{\bibinfo{volume}{525}}, \bibinfo{pages}{514–528} (\bibinfo{year}{2013}).
...
\newblock \bibinfo{title}{New \rm{L}imits on \rm{A}nomalous \rm{S}pin-\rm{S}pin \rm{I}nteractions}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{125}}, \bibinfo{pages}{201802} (\bibinfo{year}{2020}).
...
\newblock \bibinfo{title}{Bound on \rm{L}orentz and $\rm{CPT}$ \rm{V}iolating \rm{B}oost \rm{E}ffects for the \rm{N}eutron}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{93}}, \bibinfo{pages}{230801} (\bibinfo{year}{2004}).
...
\newblock \bibinfo{title}{New \rm{L}imit on \rm{L}orentz-\rm{I}nvariance- and $\rm{CPT}$-\rm{V}iolating \rm{N}eutron \rm{S}pin \rm{I}nteractions \rm{U}sing a \rm{F}ree-\rm{S}pin-\rm{P}recession $^{3}\mathrm{He}$-$^{129}\mathrm{Xe}$ \rm{C}omagnetometer}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{112}}, \bibinfo{pages}{110801} (\bibinfo{year}{2014}).
...
\newblock \bibinfo{title}{Laboratory \rm{S}earch for a \rm{L}ong-\rm{R}ange $\rm{T}$-odd, $\rm{P}$-odd \rm{I}nteraction from \rm{A}xionlike \rm{P}articles \rm{U}sing \rm{D}ual-\rm{S}pecies \rm{N}uclear \rm{M}agnetic \rm{R}esonance with \rm{P}olarized $^{129}\mathrm{Xe}$ and $^{131}\mathrm{Xe}$ \rm{G}as}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{111}}, \bibinfo{pages}{102001} (\bibinfo{year}{2013}).
...
\newblock \bibinfo{title}{Search for \rm{M}onopole-\rm{D}ipole \rm{I}nteractions at the \rm{S}ubmillimeter \rm{R}ange with a $^{129}\mathrm{Xe}\text{\ensuremath{-}}^{131}\mathrm{Xe}\text{\ensuremath{-}}\mathrm{Rb}$ \rm{C}omagnetometer}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{128}}, \bibinfo{pages}{231803} (\bibinfo{year}{2022}).
...
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{57}}, \bibinfo{pages}{3125--3128} (\bibinfo{year}{1986}).
...
\newblock \bibinfo{title}{Nuclear \rm{S}pin \rm{G}yroscope \rm{B}ased on an \rm{A}tomic \rm{C}omagnetometer}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{95}}, \bibinfo{pages}{230801} (\bibinfo{year}{2005}).
...
\newblock \bibinfo{title}{Atomic \rm{E}lectric \rm{D}ipole \rm{M}oment \rm{M}easurement \rm{U}sing \rm{S}pin \rm{E}xchange \rm{P}umped \rm{M}asers of ${}^{129}\mathrm{Xe}$ and ${}^{3}\mathrm{He}$}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{86}}, \bibinfo{pages}{22–25} (\bibinfo{year}{2001}).
...
\newblock \bibinfo{title}{New \rm{L}imit on the \rm{P}ermanent \rm{E}lectric \rm{D}ipole \rm{M}oment of $^{129}\mathrm{Xe}$ using $^{3}\mathrm{He}$ \rm{C}omagnetometry and \rm{SQUID} \rm{D}etection}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{123}}, \bibinfo{pages}{143003} (\bibinfo{year}{2019}).
...
\newblock \bibinfo{title}{Measurement of the \rm{P}ermanent \rm{E}lectric \rm{D}ipole \rm{M}oment of the \rm{N}eutron}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{124}}, \bibinfo{pages}{081803} (\bibinfo{year}{2020}).
...
\newblock \bibinfo{title}{Constraints on \rm{S}pin-\rm{D}ependent \rm{S}hort-\rm{R}ange \rm{I}nteraction between \rm{N}ucleons}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{111}}, \bibinfo{pages}{100801} (\bibinfo{year}{2013}).
...
\newblock \bibinfo{title}{Search for a coupling of the \rm{E}arth's gravitational field to nuclear spins in atomic mercury}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{68}}, \bibinfo{pages}{135--138} (\bibinfo{year}{1992}).
...
\newblock \bibinfo{title}{Search for \rm{S}pin-\rm{D}ependent \rm{G}ravitational \rm{I}nteractions at \rm{E}arth \rm{R}ange}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{130}}, \bibinfo{pages}{201401} (\bibinfo{year}{2023}).
...
\newblock \bibinfo{title}{Search for \rm{A}xionlike \rm{D}ark \rm{M}atter with a \rm{L}iquid-\rm{S}tate \rm{N}uclear \rm{S}pin \rm{C}omagnetometer}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{122}}, \bibinfo{pages}{191302} (\bibinfo{year}{2019}).
...
\newblock \emph{\bibinfo{journal}{Journal of High Energy Physics}} \textbf{\bibinfo{volume}{2019}} (\bibinfo{year}{2019}).
\newblock \urlprefix\url{http://dx.doi.org/10.1007/JHEP07(2019)069}.

\bibitem{Pau2020BECComag}
\bibinfo{author}{Gomez, P.} \emph{et~al.}
\newblock \bibinfo{title}{Bose-\rm{E}instein \rm{C}ondensate \rm{C}omagnetometer}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{124}}, \bibinfo{pages}{170401} (\bibinfo{year}{2020}).
...
\newblock \bibinfo{title}{Measurement of the $g$ \rm{F}actor of \rm{G}round-\rm{S}tate $^{87}\mathrm{Sr}$ at the \rm{P}arts-per-\rm{M}illion \rm{L}evel \rm{U}sing \rm{C}o-\rm{T}rapped \rm{U}ltracold \rm{A}toms}.
\newblock \emph{\bibinfo{journal}{Physical Review Letters}} \textbf{\bibinfo{volume}{135}}, \bibinfo{pages}{193001} (\bibinfo{year}{2025}).
...
\newblock \emph{\bibinfo{journal}{Physical Review A}} \textbf{\bibinfo{volume}{102}}, \bibinfo{pages}{062805} (\bibinfo{year}{2020}).
...
\newblock \emph{\bibinfo{journal}{Nature Photonics}} \textbf{\bibinfo{volume}{19}}, \bibinfo{pages}{89–94} (\bibinfo{year}{2025}).
...
\newblock \emph{\bibinfo{journal}{The European Physical Journal D}} \textbf{\bibinfo{volume}{67}} (\bibinfo{year}{2013}).
...
\newblock \bibinfo{title}{Ytterbium \rm{NMR}:${\mathrm{yb}}^{171}$ \rm{N}uclear \rm{M}oment and \rm{Y}b \rm{M}etal \rm{K}night \rm{S}hift}.
\newblock \emph{\bibinfo{journal}{Physical Review}} \textbf{\bibinfo{volume}{133}}, \bibinfo{pages}{A881--A884} (\bibinfo{year}{1964}).
...
\newblock \bibinfo{title}{Messung der magnetischen \rm{K}erndipolmomente an freien $^{43}\mathrm{Ca}$-, $^{87}\mathrm{Sr}$-, $^{135}\mathrm{Ba}$-, $^{137}\mathrm{Ba}$-, $^{171}\mathrm{Yb}$- und$^{173}\mathrm{Yb}$-\rm{A}tomen mit optischem \rm{P}umpen}.
\newblock \emph{\bibinfo{journal}{Zeitschrift f\"{u}r Physik}} \textbf{\bibinfo{volume}{249}}, \bibinfo{pages}{205–227} (\bibinfo{year}{1972}).
...
\newblock \emph{\bibinfo{journal}{Atomic Data and Nuclear Data Tables}} \textbf{\bibinfo{volume}{90}}, \bibinfo{pages}{75–176} (\bibinfo{year}{2005}).
...
\newblock \emph{\bibinfo{journal}{Reviews of Modern Physics}} \textbf{\bibinfo{volume}{90}}, \bibinfo{pages}{025008} (\bibinfo{year}{2018}).
...
\newblock \emph{\bibinfo{journal}{Physcal Review D}} \textbf{\bibinfo{volume}{30}}, \bibinfo{pages}{130--138} (\bibinfo{year}{1984}).
...
\newblock \emph{\bibinfo{journal}{Physcal Review D}} \textbf{\bibinfo{volume}{92}}, \bibinfo{pages}{114001} (\bibinfo{year}{2015}).
...
\newblock \emph{\bibinfo{journal}{Physcal Review C}} \textbf{\bibinfo{volume}{94}}, \bibinfo{pages}{025501} (\bibinfo{year}{2016}).

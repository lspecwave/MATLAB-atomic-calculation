function operator = spin_wigner_operator(J,theta,phi)
% 矩阵输出：左上角为m1，m2最大，右下角为m1，m2最小
% wig=Wigner3j(j1,j2,j3,m1,m2,m3,~,ifcb)
...
operator=zeros(2*J+1,2*J+1);
dim=2*J+1;
for m2_index=1:dim
    m2=J-m2_index+1;
    for m1_index=1:dim
        m1=J-m1_index+1;
        operator(m2_index,m1_index)=0;
        for small_j=0:1:(2*J)
